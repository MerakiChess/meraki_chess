...
class Searcher:
def init(self, coeff_path: Optional[str]=None, ml_alpha:float=0.35)->None:
...
self.mo.history[(int(b.turn), m.to_square)] = self.mo.history.get((int(b.turn), m.to_square),0)+depthdepth
break
...
return best
def search(self,b:chess.Board,max_depth:int,time_ms:Optional)->chess.Move:
...
def find_best_move(fen_or_board: Union[str,chess.Board], depth:int=6, time_ms:Optional=2000, coeff_path:Optional=None, ml_alpha:float=0.35)->Optional[str]:
