...
def _is_endgame(b: chess.Board) -> bool:
    return _non_pawn_material(b) <= 1300
...
class Searcher:
    ...
    def evaluate(self, b: chess.Board) -> int:
        if self.coeff_path:
            return evaluate_board_with_ml(b, self.coeff_path, self.ml_alpha)
        return eval_hc(b)
    ...
    def quiescence(self, b: chess.Board, alpha: int, beta: int, ply: int) -> int:
        ...
        for m in self.mo.order(b, noisy, None, ply):
            b.push(m)
            sc = -self.quiescence(b, -beta, -alpha, ply + 1)
            b.pop()
            if sc >= beta:
                return beta
            if sc > alpha:
    ...
    def search(self, b: chess.Board, max_depth: int, time_ms: Optional[int]) -> chess.Move:
        ...
        for d in range(1, max_depth + 1):
            ...
            if e and e.best_move:
                best = e.best_move
            if best is None:
                best = next(iter(b.legal_moves), None)
            if best is None:
                break
